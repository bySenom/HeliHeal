...
- An experimental read-only ProcTracker can identify Infusion of Light through Blizzard's registered Buff Bar Cooldown Viewer item and expose its non-secret `IsActive()` state. It does not yet alter rotation scoring; `/hh iol` reports the matched item and `/hh cdmbuffs` performs one diagnostic pool scan.
