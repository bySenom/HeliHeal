...
- A confirmed Beacon of Virtue cast starts a local nine-second window. In Raid AoE mode, HeliHeal places Virtue before ready burst cooldowns, then prioritizes Divine Toll and Ringing-enhanced Aura Mastery inside that window. Mythic+ keeps those cooldowns separated.
- Divine Favor and Divine Overload are detected and their reliable Holy Light cast-time, healing and mana modifiers are shown in the configured priority list. Redesigned Rising Sunlight is also detected, but its Beacon-health scaling cannot legally drive custom combat recommendations.
- Divine Resonance tracks the deterministic Holy Shock timings at five, ten and fifteen seconds after a confirmed Divine Toll, Holy Prism or Holy Armament trigger. At four Holy Power, a spender is promoted shortly before the next tick to avoid a predictable overcap; the actual automatic Holy Power gain remains subject to Blizzard's permitted post-cast resource reconciliation.
...
- Holy Armaments has a 60-second two-charge base recharge. Quickened Invocation reduces it by 15 seconds to 45; Forewarning additionally reduces that remaining recharge by 20% to 36 seconds when both talents are selected. Whenever a locally known or CDM-observed Infusion charge is consumed, Valiance advances the shared running recharge by three seconds. It does not alter active Holy Bulwark or Sacred Weapon effect durations.
...
- Judgment uses its current six-second base cooldown. Confirmed Holy Shock and Crusader Strike casts reduce its running cooldown by 1.5 seconds through Crusader's Might; confirmed Shield of the Righteous casts reduce Holy Shock by two seconds.
...
- Recharge reductions first settle any naturally completed charge. Snapshot charge ledgers include the resolved recharge duration, cumulative applied reduction seconds and reduction event count (since runtime reset).
- Talent data reviewed on 2026-09-15: [Quickened Invocation](https://www.wowhead.com/spell=379391/quickened-invocation) subtracts 15 seconds, [Forewarning](https://www.wowhead.com/spell=432804/forewarning) reduces cooldown by 20%, and [Valiance](https://www.wowhead.com/spell=432919/valiance) subtracts three seconds on Infusion consumption. [Laying Down Arms](https://www.wowhead.com/spell=432866/laying-down-arms) grants Infusion and reduces Lay on Hands, not Armaments directly. [Divine Inspiration](https://www.wowhead.com/spell=432964/divine-inspiration) manifests a random Armament rather than reducing its recharge. The combined flat/percent order remains the local model and needs client comparison; tooltip text alone does not establish modifier ordering.
- If the CDM hides Infusion before a consumer success arrives, a visibility observation from the last 0.3 seconds can support one consumption. It never keeps the HUD proc active. This bounded event-order estimate cannot distinguish an expiration immediately followed by a consumer; it is not an authoritative stack or cooldown read. Unreadable visibility is unknown rather than inactive.
- Generic failed casts never subtract charges. Successful casts still apply their effects when the local estimate is empty. Such a cast disproves the old recharge deadline: retain a conservative zero post-cast count and start a new local recharge at the success time. This prevents a phantom ready charge but cannot reconstruct missed earlier reductions. Repeated readable cast GUIDs are ignored for 120 seconds across confirmation paths.
- A read-only ProcTracker uses a dedicated Blizzard Tracked Bar containing only Infusion of Light. The cached item's `IsShown()` state synchronizes the local Infusion model and rotation, so random Holy Shock and Judgment procs are now recognized. Because CDM exposes presence rather than stack count, confirmed consumers and a short post-cast recheck preserve two-charge behavior. `/hh iol` reports the matched item and `/hh cdmbuffs` performs one diagnostic pool scan.
...
- Divine Spurs reduces Divine Steed's locally tracked recharge from 45 to 36 seconds; Cavalier still controls its second charge. The shorter 1.8-second movement duration is documented in the FAQ because the support strip represents readiness rather than buff uptime.
