...
- An experimental read-only ProcTracker uses a dedicated Blizzard Tracked Bar containing only Infusion of Light. The cached item's `IsShown()` state is the proc signal, so the Blizzard viewer may remain visually suppressed through its normal configuration or an alpha/off-screen layout. It does not yet alter rotation scoring; `/hh iol` reports the matched item and `/hh cdmbuffs` performs one diagnostic pool scan.
