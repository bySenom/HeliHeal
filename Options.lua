...
function HeliHeal:BuildProcsPage(parent)
    local page = CreateFrame("Frame", nil, parent)
    page:SetAllPoints()
    setPageHeader(page, "Background Procs", "Individual Blizzard Tracked Buff or Bar sources. No separate cooldown HUD.")
    local hint = text(page, "Existing Tracked Buff icons stay in place for Ellesmere. New hidden sources are assigned to Bars.\nEach proc is checked separately. UNKNOWN means no reliable observation; TRACK retries assignment.", 11, C.muted)
    ...
    function page:RefreshProcs()
        ...
        for index, entry in ipairs(tracker:GetProcEntries()) do
            ...
            if not row then
                ...
                row.track = createButton(row, "TRACK", 80, 28)
                row.track:SetPoint("RIGHT", row.remove, "LEFT", -8, 0)
                row.track:SetScript("OnClick", function()
                    local _, reason = tracker:RegisterProcWithBlizzard(row.key)
                    feedback:SetText(reason)
                    page:RefreshProcs()
                end)
                row.status:SetPoint("RIGHT", row.track, "LEFT", -8, 0)
    ...
    add:SetScript("OnClick", function()
        local ok, _, reason = HeliHeal.ProcTracker:AddProc(input:GetText())
        feedback:SetText(ok and (reason or "Added") or "Invalid / duplicate ID, or limit reached")
